...
class Ghost {
  ...
  reset() {
    ...
    this.scene.style.left = Math.floor(Math.random() * ($('.ghost-area').offsetWidth - 140)) + 'px';
    this.scene.style.top = Math.floor(Math.random() * ($('.ghost-area').offsetHeight - 160)) + 'px';
    this.scene.classList.add('descend');
    this.shadow.classList.add('disappear');
    $('.ghost-area').append(this.scene);
    this.enter();
  }
...
let ghost = new Ghost($('.scene-container'));
